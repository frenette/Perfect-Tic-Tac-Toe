...
public class TicTacToeGame extends OurObservable {
	...
	for (int row = 0; row < 3; row++) {
		...
		if (board[row][column] == '_') {
		    ...
		    availablePositions.add(new Point(row, column));
		}
	    }
	}
    ...
    /*
     * mark the position in the board but don't notify observer
     */
    public void minimaxChoose(int row, int column) {
	if (moveNumber % 2 == 0) {
	    nextChar = 'X';
	} else {
	    nextChar = 'O';
